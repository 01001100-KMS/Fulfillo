...
// ============================================================
//  Fulfillo — DummyData.h
//  Data awal yang di-load waktu program pertama jalan.
//  Hapus atau ganti isi ini kalau mau data beda.
// ============================================================

inline void loadDummyBarang(BST& bst) {
    //         id   nama                  kategori      stok   harga     minStok
    bst.insert({1,  "Aki Forklift",       "Elektrikal",  3,    450000,   2});   // low stock
    ...
    bst.insert({5,  "Forklift Pallet",    "Alat Berat",  2,    8500000,  1});   // low stock
    bst.insert({6,  "Gloves Karet",       "APD",         150,  12000,    30});
    bst.insert({7,  "Hand Truck Besi",    "Alat Angkut", 8,    320000,   3});
    bst.insert({8,  "Isolasi Coklat",     "Packaging",   4,    7500,     20});  // low stock
    ...
    bst.insert({17, "Selotip Bening",     "Packaging",   2,    8000,     15});  // low stock
...
inline void loadDummyUser(UserManager& um) {
    // Default: admin & staff1 sudah ada dari constructor UserManager
    // Tambahan:
    um.tambahUser("Heiza",    "019",  ADMIN);
    um.tambahUser("Arya",    "022",  STAFF);
    um.tambahUser("Unknown", "Unknown123",  STAFF);
}
